...
// src/components/code/games/farm/PregnancyAnimation.tsx
//
// 임신 진행률(0~1)에 따라 태아 성장 SVG 를 해당 프레임으로 freeze.
// 원본: leimapapa — https://codepen.io/leimapapa/pen/YzoEaEd (MIT License)
//   © 2026 leimapapa. SVG path 애니메이션 데이터를 게임용으로 재색상/재구성.
//
// 동작: 내부 <animate dur="100s" fill="freeze"> 들에 setCurrentTime(100*progress)
//       + pauseAnimations() 를 걸어 정지 프레임을 보여준다.
//       progress 가 매 틱 갱신되면 (FarmInteriorPage clock.tick) 부드럽게 자람.
//
// 색: 모달 톤 — 크림 배경 + 핑크 태아.

import { useEffect, useRef } from "react";

const FETUS = "#E48AAB";       // 태아 채움 (핑크)
const FETUS_LINE = "#9C4368";  // 윤곽선
const WOMB_BG = "#FBE8EE";     // 자궁 배경 (연한 핑크크림)
const WOMB_LINE = "rgba(156,67,104,0.25)";

export default function PregnancyAnimation({
  progress,
  size = 180,
}: {
  progress: number; // 0~1
  ...
  useEffect(() => {
    ...
    const animos = svg.querySelectorAll("animate");
    animos.forEach((a) => {
      const dur = parseFloat(a.getAttribute("dur") || "100") || 100;
      let t = dur * p;
      if (t <= 0) t = 0.01;
      if (t >= dur) t = dur - 0.01;
      a.ownerSVGElement?.setCurrentTime(t);
    });
    svg.pauseAnimations();
  }, [progress]);
  ...
  return (
    <div
      style={{
        ...
        borderRadius: "50%",
        background: WOMB_BG,
        border: `1px solid ${WOMB_LINE}`,
        overflow: "hidden",
        position: "relative",
        boxShadow: "inset 0 2px 12px rgba(156,67,104,0.12)",
      }}
    >
      <svg
        ref={svgRef}
      ...
      >
        ...
        <path
          ...
          fill="rgba(156,67,104,0.06)"
        />
        <g className="fetus-pulse" fill={FETUS} stroke={FETUS_LINE} strokeLinecap="round" strokeLinejoin="round" strokeWidth="0.5">
          ...
          <path className="eyeEar" fill={FETUS_LINE} stroke="none" d="">
      ...
      <style>{`
        .fetus-pulse {
          ...
          animation: fetusPulse 1.2s ease-in-out infinite, fetusSway 4.2s ease-in-out infinite;
        }
        @keyframes fetusPulse {
          0%, 100% { transform: scale(1); }
          50%      { transform: scale(1.025); }
        }
        @keyframes fetusSway {
          0%, 100% { transform: translate(0, 0); }
          25%      { transform: translate(-1%, 0.9%); }
          50%      { transform: translate(0.5%, -0.2%); }
          75%      { transform: translate(-0.2%, 1%); }
        }
      `}</style>
    </div>
  );
